...
class Facade {
  ...
  async findWithPagination({ perPage, nowPage }) {
    const items = await this.Model.find({}, null, { skip: (nowPage - 1) * perPage, limit: perPage })
    const total = await this.Model.find({}, null).countDocuments()
    return { items, total }
  }
  async paginate({ items, perPage, nowPage }) {
    ...
    return {
      ...
      items: items.slice((nowPage - 1) * perPage, nowPage * perPage)
    }
  }
  findOne(...args) {
    return this.Model
